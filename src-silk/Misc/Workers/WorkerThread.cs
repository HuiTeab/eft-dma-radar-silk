...
namespace eft_dma_radar.Silk.Misc.Workers
{
    ...
    internal sealed class WorkerThread : IDisposable
    {
        ...
        private void Worker()
        {
            ...
            {
                // Snapshot the cadence once per cycle so a runtime change to
                // SleepDuration (e.g. from within PerformWork) is honored on the
                // next cycle, including crossing the no-sleep (zero) boundary.
